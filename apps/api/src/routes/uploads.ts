...
async function storeImage(
  ...
  if (token) {
    const pathname = `kolbo/${folder}/${uuid()}${ext}`;
    try {
      const blob = await put(pathname, buffer, {
        access: blobAccess,
        token,
        contentType: mimetype || `image/${ext.replace(".", "")}`,
      });
      ...
      if (
        ...
        throw ApiError.badRequest(
          "This Vercel Blob store is Private but the app defaults to public uploads. " +
            "Recommended: create a Public Blob store (Vercel → Storage → Blob) and connect it to this project. " +
            "Alternatively set env BLOB_ACCESS=private (images will not load in <img> on public pages without a proxy).",
        );
      }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await storeImage(req.file.buffer, ext, req.file.mimetype, "images", res);
  }),
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await storeImage(req.file.buffer, ext, req.file.mimetype, "avatars", res);
  }),
);
