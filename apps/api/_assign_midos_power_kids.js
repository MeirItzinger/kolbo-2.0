...
(async () => {
  ...
  let category = await p.category.findFirst({
    where: {
      ...
      OR: [
        { slug: { equals: "midos-power-kids", mode: "insensitive" } },
        { name: { equals: "Midos Power Kids", mode: "insensitive" } },
      ],
    },
  });
  ...
  if (!category) {
    category = await p.category.create({
      data: {
        channelId: channel.id,
        name: "Midos Power Kids",
        slug: "midos-power-kids",
        sortOrder: 0,
        isActive: true,
      },
    });
  ...
  const videos = await p.video.findMany({
    where: {
      channelId: channel.id,
      title: { contains: "Midos Power Kids", mode: "insensitive" },
    },
    select: { id: true, title: true },
  });

  console.log("Videos matching title contains 'Midos Power Kids':", videos.length);
