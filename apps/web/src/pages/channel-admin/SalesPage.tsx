...
import { adminListSalesByChannel, adminGetChannel } from "@/api/admin";
...
export default function ChannelAdminSalesPage() {
  ...
  const salesQuery = useQuery({
    queryKey: ["channel-admin", channelId, "sales", { page, search: debouncedSearch, typeFilter }],
    queryFn: () =>
      adminListSalesByChannel(channelId!, {
        page,
        perPage: 25,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <nav className="mb-2 flex items-center gap-2 text-sm text-surface-400">
            <Link
              to={`/channel-admin/${channelId}`}
              className="hover:text-surface-200"
            >
              {channelQuery.data?.name ?? "Channel"}
            </Link>
            <ChevronRightIcon className="h-4 w-4" />
            <span className="text-surface-200">Sales</span>
          </nav>
          <h1 className="text-2xl font-bold text-white">Sales</h1>
          <p className="mt-1 text-sm text-surface-400">
            Transactions for this channel (subscriptions, rentals, purchases)
          </p>
        </div>
        {meta && (
      ...
      {salesQuery.isLoading ? (
        ...
        <div className="rounded-lg border border-surface-700 bg-surface-800/50 py-16 text-center">
          ...
          <p className="mt-1 text-sm text-surface-400">
            Transactions for this channel will appear here.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-700 bg-surface-800/80">
                <th className="px-4 py-3 text-left font-medium text-surface-300">Date</th>
                <th className="px-4 py-3 text-left font-medium text-surface-300">Invoice #</th>
                <th className="px-4 py-3 text-left font-medium text-surface-300">Customer</th>
                <th className="px-4 py-3 text-left font-medium text-surface-300">Email</th>
                <th className="px-4 py-3 text-left font-medium text-surface-300">Type</th>
                <th className="px-4 py-3 text-left font-medium text-surface-300">Description</th>
                <th className="px-4 py-3 text-right font-medium text-surface-300">Amount</th>
                <th className="px-4 py-3 text-center font-medium text-surface-300">Status</th>
              </tr>
            </thead>
  ...
  return (
    <tr className="transition-colors hover:bg-surface-800/40">
      ...
      <td className="whitespace-nowrap px-4 py-3">
        {tx.invoiceNumber ? (
          <span className="font-mono text-xs text-surface-200">{tx.invoiceNumber}</span>
        ) : tx.stripeInvoiceId ? (
          <span className="font-mono text-xs text-surface-400">{tx.stripeInvoiceId}</span>
        ) : (
      ...
      <td className="max-w-[240px] truncate px-4 py-3 text-surface-300">
        {tx.description || tx.channelName || tx.bundleName || tx.videoTitle || "—"}
      </td>
      <td className="whitespace-nowrap px-4 py-3 text-right font-medium text-white">
        {formatCurrency(tx.amount, tx.currency)}
      </td>
      <td className="whitespace-nowrap px-4 py-3 text-center">
        <Badge variant={isFailed ? "destructive" : "success"}>{statusLabel}</Badge>
      </td>
    </tr>
  );
}
